...
// Hardcoded default admin recipient (can also be overridden by ADMIN_EMAIL env var)
// The user will NEVER see this admin email address.
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || "michaelmidhun100@gmail.com";
...
<body>
  <div class="container">
    ...
    // Dispatch both Admin (with PDF) and User confirmation emails concurrently for maximum speed
    const [adminSendInfo, userSendInfo] = await Promise.all([
      transporter.sendMail(adminMailOptions),
      transporter.sendMail(userMailOptions),
    ]);

    const adminPreviewUrl = nodemailer.getTestMessageUrl(adminSendInfo) || undefined;
    const userPreviewUrl = nodemailer.getTestMessageUrl(userSendInfo) || undefined;
    console.log(`[Admin Email] Dispatched to ${ADMIN_EMAIL} with PDF attachment. MessageId: ${adminSendInfo.messageId}`);
    console.log(`[User Email] Dispatched to ${recipientEmail}. MessageId: ${userSendInfo.messageId}`);

    res.json({
      success: true,
      messageId: userSendInfo.messageId,
      ...
      userPreviewUrl,
      message: hasLiveSmtp
        ? `Application submitted. User confirmation dispatched to ${recipientEmail}, and admin notification with PDF attachment dispatched to secretariat.`
        : `Email dispatched in sandbox mode. User Preview: ${userPreviewUrl || 'N/A'}, Admin Preview: ${adminPreviewUrl || 'N/A'}`,
    });
  } catch (error: any) {
